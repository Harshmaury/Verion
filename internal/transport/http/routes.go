...
func (g *Gateway) registerRoutes(mux *http.ServeMux) {
	auth := RequireAuth(g.tokenSvc, g.sessionStore)
	...
	mux.Handle("POST /v1/auth/logout", auth(http.HandlerFunc(g.sessionHandler.Logout)))
	mux.Handle("GET /v1/auth/sessions", auth(http.HandlerFunc(g.sessionHandler.ListSessions)))
	mux.Handle("DELETE /v1/auth/sessions/{session_id}", auth(http.HandlerFunc(g.sessionHandler.RevokeSession)))
...
func (g *Gateway) handleCreateTenant(w http.ResponseWriter, r *http.Request) {
	...
	result, err := g.tenantSvc.CreateTenant(r.Context(), identity.CreateTenantInput{
		Name: req.Name, Slug: req.Slug, Tier: identity.TenantTier(req.Tier), DataRegion: req.DataRegion,
	})
...
func (g *Gateway) handleCreateIdentity(w http.ResponseWriter, r *http.Request) {
	...
	input := identity.CreateIdentityInput{
		TenantID: req.TenantID, Type: identity.IdentityType(req.Type),
		DisplayName: req.DisplayName, Handle: req.Handle, Attributes: req.Attributes,
	}
	if req.CreatedBy != "" { input.CreatedBy = &req.CreatedBy }
...
func (g *Gateway) handleGetIdentity(w http.ResponseWriter, r *http.Request) {
	...
	if tenantID == "" { writeError(w, http.StatusBadRequest, "tenant_id query parameter required"); return }
...
func (g *Gateway) handleUpdateIdentity(w http.ResponseWriter, r *http.Request) {
	...
	input := identity.UpdateIdentityInput{TenantID: req.TenantID, ID: id, DisplayName: req.DisplayName, Attributes: req.Attributes, Version: req.Version}
	if req.ActorID != "" { input.ActorID = &req.ActorID }
...
func (g *Gateway) handleSuspendIdentity(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	var req struct{ TenantID string `json:"tenant_id"`; ActorID string `json:"actor_id"` }
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
	if err := g.identitySvc.SuspendIdentity(r.Context(), req.TenantID, id, req.ActorID); err != nil { writeServiceError(w, err); return }
	writeJSON(w, http.StatusOK, map[string]string{"status": "suspended"})
}
...
func (g *Gateway) handleReactivateIdentity(w http.ResponseWriter, r *http.Request) {
	...
	var req struct{ TenantID string `json:"tenant_id"`; ActorID string `json:"actor_id"` }
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
	if err := g.identitySvc.ReactivateIdentity(r.Context(), req.TenantID, id, req.ActorID); err != nil { writeServiceError(w, err); return }
	writeJSON(w, http.StatusOK, map[string]string{"status": "active"})
}
...
func (g *Gateway) handleDeactivateIdentity(w http.ResponseWriter, r *http.Request) {
	...
	var req struct{ TenantID string `json:"tenant_id"`; ActorID string `json:"actor_id"` }
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
	if err := g.identitySvc.DeactivateIdentity(r.Context(), req.TenantID, id, req.ActorID); err != nil { writeServiceError(w, err); return }
...
func (g *Gateway) handleGenerateKey(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
	input := identity.GenerateKeyInput{TenantID: req.TenantID, IdentityID: req.IdentityID, KeyType: identity.KeyType(req.KeyType), Purpose: identity.KeyPurpose(req.Purpose)}
	if req.ActorID != "" { input.ActorID = &req.ActorID }
...
func (g *Gateway) handleRotateKey(w http.ResponseWriter, r *http.Request) {
	...
	var req struct{ TenantID string `json:"tenant_id"`; IdentityID string `json:"identity_id"`; ActorID string `json:"actor_id"` }
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
...
func (g *Gateway) handleRevokeKey(w http.ResponseWriter, r *http.Request) {
	...
	var req struct{ TenantID string `json:"tenant_id"`; ActorID string `json:"actor_id"` }
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeError(w, http.StatusBadRequest, "invalid request body"); return }
	if err := g.keySvc.RevokeKey(r.Context(), req.TenantID, keyID, req.ActorID); err != nil { writeServiceError(w, err); return }
